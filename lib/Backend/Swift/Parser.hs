...
module Backend.Swift.Parser (terminalCount,
               categoryCount,
               lotsawaRule,
               recognizingSym,
               LotsawaRule (..),
...
instance Show LotsawaRule where
    show (Backend.Swift.Parser.Rule i ints) = "(lhs: "
                                            ++ show i
                                            ++ ", rhs: "
                                            ++ show ints
                                            ++ ")"
...
instance Show LotsawaGrammar where
    show (Grammar {rules, recognizing}) = unlines $
        ("var grammar: Grammar<Int16> = Grammar(recognizing: "
            ++ show recognizing ++ ")")
...
recognizingSym cfg = do
    ...
    (M.lookup parseCat <$> ask) <&> \case Just i -> (parseCat, i)
